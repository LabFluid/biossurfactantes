...
ux, uy, phi = np.ones((nx, ny)) * 3.0e-5, np.ones((nx,ny)) * 2.0e-5, np.ones((nx, ny)) * 0.2
...
def flux_x(Sw, nD, i, j):
    return np.array([ux[i,j] * fw(Sw, nD),
                     nD * fg(Sw, nD) * ux[i,j]])

def flux_y(Sw, nD, i, j):
    return np.array([uy[i,j] * fw(Sw, nD),
                     nD * fg(Sw, nD) * uy[i,j]])
...
def eigvals(Sw, nD, u_face, i, j):
    eps = 1e-8
    dfw_dSw = (fw(Sw + eps, nD) - fw(Sw - eps, nD)) / eps
    l1 = u_face * dfw_dSw / phi[i,j]
    Sg_val = Sg(Sw)
    l2 = np.where(Sg_val > 1e-10, (u_face * fg(Sw, nD)) / (phi[i,j] * Sg_val), 0.0)
    return l1, l2
...
def KNP_flux_x(S):
    ...
    for i in range(n_faces):
        for j in range(ny):
            ...
            l1_L, l2_L = eigvals(Sw_L, nD_L, u_face, i, j)
            l1_R, l2_R = eigvals(Sw_R, nD_R, u_face, i, j)
            
            a_plus = np.maximum(0, l1_L,np.maximum(l2_L, l1_R, l2_R))
            a_minus = np.minimum(0, l1_L, np.minimum(l2_L, l1_R, l2_R))

            hl = flux_x(Sw_L, nD_L, i, j)
            hr = flux_x(Sw_R, nD_R, i, j)
            
            s_l = S_L[:, i, j]
            s_r = S_R[:, i, j]

            denom = (a_plus - a_minus)
            if np.all(abs(denom) < 1e-12):
                H[:, i, j] = 0.5 * (hl + hr)
            else:
...
def KNP_flux_y(S):
    ...
    for i in range(nx):
        for j in range(n_faces):
            ...
            hl = flux_y(Sw_L, nD_L, i, j)
            hr = flux_y(Sw_R, nD_R, i, j)
...
def compute_rhs(S):
    ...
    f_out = np.zeros((2, ny))
    for j in range(ny):
        f_out[:, j] = flux_x(Sw_out[j], nD_out[j], nx-1, j)
...
def dt_cfl():
    ...
    v_x_field = np.abs(ux) / (phi + 1e-15)
    v_y_field = np.abs(uy) / (phi + 1e-15)

    max_v_x = np.max(v_x_field) * max_df_dS
    max_v_y = np.max(v_y_field) * max_df_dS

    if max_v_x < 1e-10 and max_v_y < 1e-10:
        return 1e-3

    dt_x = dx / max_v_x if max_v_x > 1e-10 else 1.0e10
    dt_y = dy / max_v_y if max_v_y > 1e-10 else 1.0e10

    dt_calc = cfl * min(dt_x, dt_y)
...
plt.pcolormesh(X, Y, S[0], cmap='jet', vmin=Swc, vmax=1.0)
