...
public:
	...
	void drawPoint(Point& p) {
		...
		if (p.fill) {
			...
			DrawCircle(screenSpace.x, screenSpace.y, p.r, p.color);
		}
	}
...
public:
	...
	void setFunction(const std::function<double(double, double)>& func) {
		f = func;
	}
	...
	void teste() {
		...
		for (double x = 0.0; x <= 1.0 + step * 0.5; x += step) {

			vector<Point3D> v;

			for (double z = 0.0; z <= 1.0 + step * 0.5; z += step) {
				double y = f(x, z);

				maxValue = std::max(maxValue, y);
				minValue = std::min(minValue, y);

				v.push_back(Point3D((x - 0.5) * 20.0, y, (z - 0.5) * 20.0));
			}
		...
		olc::Pixel colorLow = olc::DARK_BLUE;

		std::vector<std::tuple<Point, Point, Point, double>> triangles(0);

		for (size_t i = 0; i < heightMap.size() - 1; ++i) {
			for (size_t j = 0; j < heightMap[0].size() - 1; ++j) {

				Point3D p1 = heightMap[i][j]; p1.y /= maxValue * 0.05;
				Point3D p2 = heightMap[i + 1][j]; p2.y /= maxValue * 0.05;
				Point3D p3 = heightMap[i + 1][j + 1]; p3.y /= maxValue * 0.05;
				Point3D p4 = heightMap[i][j + 1]; p4.y /= maxValue * 0.05;

				p1 = pointToCameraSpace(p1);
				p2 = pointToCameraSpace(p2);
				p3 = pointToCameraSpace(p3);
				p4 = pointToCameraSpace(p4);
			...
			//	vector<vector<Point3D>> tris = clipToNearPlane({ { p1, p2, p3 }, { p1, p4, p3 } }, nearPlane);
				...
				for (size_t i = 0; i < tris.size(); ++i) {
					...
					double depth = (tris[i][0].z + tris[i][1].z + tris[i][2].z) / 3.0;
					...
					for (size_t j = 0; j < newTris.size(); ++j) {
						...
						triangles.emplace_back(newTris[j][0], newTris[j][1], newTris[j][2], depth);
						continue;
						...
						DrawTriangle(newTriP1, newTriP2, newTriP3, olc::VERY_DARK_GREY);
					}
				}
			}
		}

		// Painter's algorithm. It should work perfectly since the triangles in my grid have no overlap
		std::sort(triangles.begin(), triangles.end(), [](const auto& a, const auto& b) {
			return std::get<3>(a) > std::get<3>(b);
		});

		for (size_t i = 0; i < triangles.size(); ++i) {
			std::vector<Point> newTris = { std::get<0>(triangles[i]), std::get<1>(triangles[i]), std::get<2>(triangles[i]) };
			olc::vf2d newTriP1(newTris[0].x, newTris[0].y);
			olc::vf2d newTriP2(newTris[1].x, newTris[1].y);
			olc::vf2d newTriP3(newTris[2].x, newTris[2].y);

			olc::vf2d z(0, 0);

			FillTexturedTriangle({ newTriP1, newTriP2, newTriP3 }, { z, z, z }, { newTris[0].color, newTris[1].color, newTris[2].color }, nullptr);
			DrawTriangle(newTriP1, newTriP2, newTriP3, olc::DARK_GREY);
		}
